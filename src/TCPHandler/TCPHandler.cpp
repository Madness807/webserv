			...
			if (FD_ISSET(i, &copy))
			{
				...
				for(std::vector<Server>::iterator it = _servers.begin(); it != _servers.end(); ++it)
				{
					if (i != it->getServerSocket())
					{
						...
						std::cout << "fdclient clients[i].getSocketClient()" << newClient.getSocketClient() << " i : " << i << std::endl;
						//reading = recv(*_fdClients.begin(), tmp, sizeof(tmp), 0);
						...
						if (it->getReading() <= 0)
						{
							...
							{
								if (*it == newClient.getSocketClient())
								{
						...
						{
							newClient.setBuffer(tmp);
							it->sendToClient(newClient.getSocketClient(), newClient.getBuffer().c_str(), it->getReading());
							std::cout << "getBuffer: " << newClient.getBuffer() << "I : " << i << std::endl;
							std::cout << "socket client: " << newClient.getSocketClient() << " socketServer associated with : " << newClient.getServerSocketAssociated() << std::endl;
						}
						break;
					}
				}
